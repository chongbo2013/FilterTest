...
public class Image {
    ...
    private void updateColorArray(){
        ...
        for (int y = 0; y < height; y++){
            for (int x = 0; x < width; x++){
                ...
                colorArray[index] = (a << 24) | (b << 16) | (g << 8) | r;
            }
        }
    }
     ...
     * @param c2
     */
    public void setPixelColor(int x, int y, int c0, int c1, int c2,int alpha){
	    int rgbcolor = (alpha << 24) + (c0 << 16) + (c1 << 8) + c2;
    ...
    public int getRComponent(int x){
    ...
    public int getGComponent(int x){
    ...
    public int getBComponent(int x){
        return (x & 0x000000FF);
    }
    public int getAComponent(int x, int y){
        return (getColorArray()[((y*width+x))] & 0xFF000000)>>>24;
    }
